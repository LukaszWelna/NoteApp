...
export default function Home() {
    ...
    useEffect(() => {
        let isValid = (registerPassword.length >= 8) ? true : false;
        setRegisterValidPassword(isValid);
        let isMatch = registerPassword === registerConfirmPassword;
    ...
    useEffect(() => {
        let result = EMAIL_REGEX.test(registerEmail);
    ...
    async function handleRegisterSubmit(event) {
        ...
        try {
            ...
            if (!e?.response) {
                ...
                let errorsData = e.response.data.errors;
                let errors = [];
                    ...
                    <div className='d-flex flex-column justify-content-center h-custom-2 w-100 pt-4 pb-1 px-3 center-text'>
                        ...
                        <form onSubmit={handleLoginSubmit}>
                            ...
                            <MDBInput wrapperClass='mb-4 w-100' label='Email address' id='formLoginEmail'
                                type='email' size="lg" onChange={handleLoginEmail} value={loginEmail} required />

                            <MDBInput wrapperClass='mb-4 w-100' label='Password' id='formLoginPassword'
                                type='password' size="lg" onChange={handleLoginPassword} value={loginPassword} required/>
                                ...
                                <MDBRow className='justify-content-center mx-0'>
                                    ...
                                    <MDBCol className='login-div rounded-4'>
                                        ...
                                        <div className='d-flex flex-column justify-content-center h-custom-2 w-100 pt-4 pb-1 px-3 center-text'>
                                            ...
                                            <MDBInput wrapperClass='mb-4 w-100' label='First name' id='formSignupFirstName' type='text' size="lg" onChange={(e) => setRegisterFirstName(e.target.value)} required />
                                            <MDBInput wrapperClass='mb-4 w-100' label='Last name' id='formSignupLastName' type='text' size="lg" onChange={(e) => setRegisterLastName(e.target.value)} required />
                                            <MDBInput wrapperClass='mb-4 w-100' label='Email address' id='formSignupEmail' type='email' size="lg" onChange={(e) => setRegisterEmail(e.target.value)} required />
                                            <p className={(!registerValidEmail && registerEmail) ? 'validation-errors' : 'not-display'}>Email address is not valid.</p>
                                            <MDBInput wrapperClass='mb-4 w-100' label='Password' id='formSignupPassword' type='password' size="lg" onChange={(e) => setRegisterPassword(e.target.value)} required />
                                            ...
                                            <MDBInput wrapperClass='mb-4 w-100' label='Confirm password' id='formSignupConfirmPassword' type='password' size="lg" onChange={(e) => setRegisterConfirmPassword(e.target.value)} required />
